...
/*!
 * Chooses a Dynamic Content variation and returns the content as a JSON object.
 *
 *  @param smartObjectID The smartObjectID
 *
 *  @return Yes if the params are not nil or empty
 */
 ...
 * @param normalizedScores - True for normalized scores
 *
 * @return NSDictionary with the affinity data
 */
 ...
 * @param pushID : The user’s push token.
 ...
 * @param time : An array of one or more values defining the timeframe going back
 * @param interest : Determines if viewing data, purchase data, or both are returned
 * @param includeData : Determines whether or not to display product attributes and details such as color, style, etc
 * @param delegate : a class instance that implements DYProductActivityDataDelegate Protocol
 *
 * @return valid/not valid
 */
